...
function initializeListeners() {
    const q = query(collection(db, "Nextcart"));
    const unsubscribe = onSnapshot(q, async (snapshot) => {
        ...
        snapshot.forEach(async (doc) => {
            ...
            try {
                const addToCartDocRef = await addDoc(collection(db, "Nextstore"), {
                    ProductUrl: doc.data().Pic,
...
document.getElementById('btn').addEventListener('click', async () => {
    ...
    deleteAllDocumentsInCollection("Nextcart");
});
